...
export default function B2BRegister() {
  ...
  const submit = async () => {
    const res = await fetch('/api/auth/register', {
      ...
      body: JSON.stringify({ name, email: mode==='email'?email: undefined, phone: mode==='phone'?phone: undefined })
    });
  ...
  return (
    <div className="container max-w-md py-10 space-y-6">
      ...
      {step==='form' ? (
        <div className="space-y-3">
          ...
          <label className="text-sm">Full Name
            <input className="w-full rounded-xl border p-2 mt-1" value={name} onChange={e=>setName(e.target.value)} />
          </label>
          ...
          <button onClick={submit} className="btn btn-primary w-full">Get OTP</button>
      ...
      <div className="text-sm text-gray-500">Already have an account? <a href="/b2b/login" className="underline">Login</a></div>
    </div>
  );
}
